    ...
    def combine_files(self, image_path, label_path, is_test):
        ...
        for f in image_path.iterdir():
            ...
            if "ReflStackFlat" in f.name:
                ...
                mask = label_path / f.name.replace("ReflStackFlat", "L").replace(
                    "tif", "png"
                )
                if trans.exists and mask.exists:
                    file_set = {"refl": f, "trans": trans, "mask": mask}
    ...
    def open_as_array(self, idx, invert=True):
        raw_stack = np.stack(
            [
                ...
                np.array(
                    standardize_image(Image.open(self.files[idx]["trans"]))
                ),  # fool it into rgb?!
            ],
            axis=2,
        )
...
train_size = round(data_len * TRAIN_SPLIT / 100)
test_size = round(data_len * (TEST_SPLIT) / 100)
